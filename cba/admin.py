...
from .models import CbaProfile, Cba
...
# D - CbaProfile Admin #
@admin.register(CbaProfile)
class CbaProfileAdmin(DjangoAdminOnlyMixin, admin.ModelAdmin):
...
# E - Cba Admin #
@admin.register(Cba)
class CbaAdmin(DjangoAdminOnlyMixin, admin.ModelAdmin):
    list_display = ['client_name_cba', 'wo_no_cba', 'created_by', 'team', 'created_at']
    list_filter = ['team', 'created_by']
    search_fields = ['client_name_cba__client_name_ctv', 'wo_no_cba', 'created_by__username']
    ordering = ['-created_at']
