...
from .models import CastingProfile, Casting
...
# D - CastingProfile Admin #
@admin.register(CastingProfile)
class CastingProfileAdmin(DjangoAdminOnlyMixin, admin.ModelAdmin):
...
class CastingAdmin(DjangoAdminOnlyMixin, admin.ModelAdmin):
    list_display = ['project_name_casting', 'location', 'created_by', 'team', 'created_at']
    list_filter = ['team', 'created_by']
    search_fields = ['project_name_casting', 'location', 'created_by__username']
    ordering = ['-created_at']
